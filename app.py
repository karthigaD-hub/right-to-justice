...
corpus = [
    section['title'] + ". " + section['description'] + " " + " ".join(section.get('keywords', []))
...
def fetch_primary_sections(crime_type):
    return [s for s in sections_data if crime_type and any(crime_type in kw.lower() for kw in s["keywords"])]
